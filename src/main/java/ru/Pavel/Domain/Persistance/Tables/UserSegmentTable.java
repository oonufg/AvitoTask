...
public class UserSegmentTable extends SegmentServiceTable {
    ...
    private PreparedStatement getDeleteUserSegmentStatement(long user_id, long segment_id) throws SQLException{
        String query =
                "UPDATE users_segments  SET action = 'removal' " +
                "WHERE user_id =? AND segment_id =? AND action = 'adding'";
    ...
    private PreparedStatement getIsUserHaveSegmentStatement(long userId, long segmentId, String action) throws SQLException{
        String query =
                "SELECT * FROM users_segments " +
                "WHERE user_id =? AND segment_id =? AND action = ?";
        PreparedStatement statement = getStatement(query);
    ...
    private PreparedStatement getUserSegmentsStatement(long user_id) throws SQLException {
        String query =
                "SELECT segments.id, segments.slug FROM users_segments " +
                "LEFT JOIN segments ON users_segments.segment_id= segments.id " +
                "WHERE users_segments.user_id = ? AND users_segments.action = 'adding'";
        PreparedStatement statement = getStatement(query);
        statement.setLong(1,user_id);
        return statement;
    }
    ...
    private PreparedStatement getUserSegmentsHistoryStatement(long user_id) throws SQLException{
        String query =
                "SELECT segments.id, segments.slug, users_segments.action, users_segments.timestamp FROM users_segments " +
                "LEFT JOIN segments ON users_segments.segment_id= segments.id " +
                "WHERE users_segments.user_id = ? ORDER BY timestamp DESC";
        PreparedStatement statement =  getStatement(query);
        statement.setLong(1, user_id);
        return statement;
    }
    ...
    private PreparedStatement deleteExpiredUserSegmentsStatement(long timestamp) throws SQLException{
        String query =
                "UPDATE users_segments  SET action = 'removal' " +
                "WHERE action <> 'removal' AND expired_timestamp <= ?";
        PreparedStatement statement = getStatement(query);
